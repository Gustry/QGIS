...
def run(item, action, mainwindow):
    ...
    if (res[0][0] < 0):
        mainwindow.infoBar.pushMessage("WARNING", f'Topology "{item.schema().name}" is registered as having a srid of {res[0]} in topology.topology, we will assume 0 (for unknown)', Qgis.Warning, mainwindow.iface.messageTimeout())
        toposrid = '0'
    else:
